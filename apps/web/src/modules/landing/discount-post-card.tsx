  ...
  return (
    <Dialog>
      <DialogTrigger asChild>
        <button className="block outline-none text-left w-full cursor-pointer">
          <div
          ...
          >
            ...
            <div className="flex-1 p-5 flex flex-col justify-between bg-gradient-to-r from-transparent to-gray-50/50 dark:to-gray-900/50">
              ...
              <div className="mt-3 pt-3 border-t border-gray-100 dark:border-gray-700/50 flex items-end justify-between">
                ...
                <div className="flex items-center gap-2 bg-[#D4AF37] text-white px-3 py-1.5 rounded-lg shadow-md shadow-[#D4AF37]/20 group-hover:scale-105 transition-transform duration-300">
      ...
      <DialogContent className="sm:max-w-[600px] max-h-[85vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="text-2xl font-bold text-[#0F172A] dark:text-white">
            {post.postTitle}
          </DialogTitle>
          <DialogDescription asChild>
            <div className="space-y-4 pt-4">
              {post.postImageUrl && (
                <div className="relative w-full h-64 rounded-xl overflow-hidden m-0">
                  <Image
                    src={post.postImageUrl}
                    alt={post.postTitle}
                    fill
              ...
              {(post.startDate || post.endDate) && (
                <div className="flex flex-wrap gap-4 py-2 border-y border-gray-100 dark:border-gray-800">
                  {post.startDate && (
                    <div className="flex items-center gap-2 text-sm text-emerald-700 dark:text-emerald-400">
                      <CalendarDays className="w-4 h-4" />
                      <span className="font-semibold">
                        Gültig ab: {formatDate(post.startDate)}
                      </span>
                    </div>
                  )}
                  {post.endDate && (
                    <div className="flex items-center gap-2 text-sm text-rose-700 dark:text-rose-400">
                      <CalendarDays className="w-4 h-4" />
                      <span className="font-semibold">
                        Gültig bis: {formatDate(post.endDate)}
                      </span>
                    </div>
                  )}
                </div>
