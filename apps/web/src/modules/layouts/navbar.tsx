...
export function Navbar({}: Props) {
  ...
  return (
    <>
      ...
      <div
        className={cn(
          "fixed top-0 left-0 w-full z-50 h-16 flex items-center justify-between px-4 lg:px-8 bg-white border-b border-gray-200 shadow-sm"
        )}
      >
        {/* Logo */}
        <div className="flex items-center">
            ...
            <Link
              ...
              className={cn(
                ...
                pathname === item.href
                  ...
                  : "text-gray-700 hover:text-[#E4BF3C]"
              )}
            >
              {item.label}
            </Link>
        ...
        <div className="flex items-center gap-3">
          <Button
            variant="outline"
            className={cn(
              "text-white bg-[#E4BF3C] hover:bg-[#f6d355] hover:text-white cursor-pointer"
            )}
            onClick={() => {
              const activeOrgId = session?.session?.activeOrganizationId;
              if (activeOrgId) {
