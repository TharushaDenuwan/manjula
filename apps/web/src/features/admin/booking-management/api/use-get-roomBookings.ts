...
export const useGetRoomBookings = (params: FilterParams) => {
  const {
    ...
    search = "",
    sort = "desc",
    paymentType = "online", // Default to "online" payments only
  } = params;
  ...
  const query = useQuery({
    ...
    queryFn: async () => {
      ...
      // Always filter to show only online payments (exclude cash)
      if (roomBookings.data) {
        const filteredData = roomBookings.data.filter(
          (booking: any) => booking.paymentType === "online"
        );
...
// Hook specifically for online payments only (this is now the default behavior)
...
// Hook for cash payments (if needed for admin purposes only)
